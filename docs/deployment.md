...
| `PROXORA_TELEMETRY` | no | Anonymous GitHub usage ping (`stats` prerelease). Default on in production. Set `0` to disable. |

WireGuard lives under **WireGuard** in the sidebar. Each Proxora is a **client of the colleague's WG server** (import their client `.conf`), then you enter **their Proxora IP** and the same **shared secret**. Share your own hosts; they do the reverse. No invite exchange. UDP 51820 is published only on the WG VMs. The sidecar uses MTU 1280 and TCP MSS clamping so Proxmox API payloads fit through the tunnel.
