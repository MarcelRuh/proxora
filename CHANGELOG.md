...
- Web, favicon, Discord avatar, README, and Android launcher use the new hex/P mark and full Proxora lockup.
...
- LXC SSH toggle parses each `/etc/ssh/sshd_config` line: uncommented `PermitRootLogin` shows **SSH ausschalten**, `#PermitRootLogin` shows **SSH einschalten**.
...
- Android release builds use R8; dead `/ws/push` Android push socket is gone.
- R8 keeps Tink/`javax.annotation` warnings from failing the signed APK build.
- In-app inbox and browser Web Push are gone. Events only go to configured Discord/SMTP/webhook channels.
...
- Android keeps the system status bar (clock, battery) and insets the WebView below it. No Proxora title bar.
...
- Standard preview and Standard skin keep the original neon palette when another skin is selected.
