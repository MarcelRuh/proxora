...
export async function handleNodeGuestFileTransfer(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  try {
    ...
    const permission = guestFilePermission(parsed.kind, parsed.mode === "upload" ? "write" : "read");
    const guest = { hostId: parsed.hostId, kind: parsed.kind, vmid };
    if (!userHasAnyPermission(session.user, [permission], parsed.hostId, guest)) throw new ForbiddenError();
