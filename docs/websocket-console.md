...
4. For QEMU, if `serial0` is missing, the proxy sets `serial0: socket` (required by termproxy).
5. Backend calls Proxmox `POST .../termproxy` and receives `{ port, ticket, user }`.
6. Backend opens `wss://{pve}/api2/json/nodes/{node}/.../vncwebsocket?port&vncticket`.
7. First upstream message: `{user}:{ticket}\n` (text frame).
8. After `OK`, the termproxy protocol is used:
...
4. Backend opens the same `vncwebsocket` path. If `generate-password` returned a password (PVE 9), RFB starts immediately and the ticket line is never sent. Older PVE still gets `{user}:{ticket}` if RFB does not arrive first. Sending the ticket into a live RFB stream makes QEMU close the socket (1006). Headless VMs (`vga=none`) are rejected; use Serial.
...
9. Keyboard uses standard RFB KeyEvent (type 4). While the VGA session is connected, keys are captured on `window` with preventDefault (Space must not scroll the page). Form fields and dialogs keep the browser keyboard.
